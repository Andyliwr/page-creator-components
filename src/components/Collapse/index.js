 ...
 * @LastEditTime: 2019-04-24 10:54:30
 */
...
export default Base(
  class CollapsePreview extends React.PureComponent {
    ...
    getHeaderFn(){
      ...
      if(imgSrc) {
        header = <div onClick={this.clickHeaderFn}><img src={imgSrc} style={{width: '100%'}} /></div>
      }else{
        let pointStyle = { color: pointColor };
        if(status) pointStyle.transform = 'rotate(180deg)';
        let contentStyle = { background: headerBackground };
        header = (
          <div className={styles['preview-header']} style={contentStyle} onClick={this.clickHeaderFn}>
            <div style={{display: 'inline-block'}} dangerouslySetInnerHTML={{__html: headerContent}}></div>
            <i style={pointStyle} className={styles['iconfont'] + ' ' +styles['preview-point']} />
          </div>
        );
      }
    ...
    getBodyFn(){
      ...
      if(imgSrc){
        body = <div onClick={this.clickHeaderFn}><img src={imgSrc} style={{width: '100%'}} /></div>
      }else{
        let bodyStyle = { background: bodyBackground };
        body = <div className={styles['preview-body']} style={bodyStyle} dangerouslySetInnerHTML={{__html: bodyContent}}></div>;
      }
      return body;
    }
    ...
    render() {
      ...
      return (
        <div data-id={id} className={styles['preview-main']}>
          {header} {body}
        </div>
      )
    }
