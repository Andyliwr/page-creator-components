 ...
 * @LastEditTime: 2019-04-18 15:18:44
 */
...
export default Base(
  class SwiperPreview extends React.PureComponent {
    constructor(props){
      super(props);
      this.state = {
        top:0, left: 0, transition: 'top 1.5s',
      }
    }
    timer = null;
    drawFinished = false;
    ...
    getStyleFn(){
      let { scrollDirection, contents, color, fontSize, lineHeight } = this.props;
    ...
    getContentListFn({contents, length, contentList, li, key}){
      for(let i = 0; i < length; i++){
        ...
        contentList.push( <li key={`${key + i}`} style={li}>{content}</li> );
      }
    }
    getContentBoxFn(){
      ...
      return (
        <ul className={styles['preview-ul']} style={ul} ref="carouselContentBox">{contentList}</ul>
      );
    }
    ...
    render() {
      let { id, speed, scrollDirection} = this.props;
      let { html } = this.state;

      let contentBox= this.getContentBoxFn();
