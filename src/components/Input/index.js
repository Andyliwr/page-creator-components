 ...
 * @LastEditTime: 2019-04-25 19:29:34
 */
...
export default Base(
  class InputPreview extends React.PureComponent {
    ...
    getInputListFn(){
      const { raduisType, isEncrypt, isRequired, contents, pinyins, inputBorderColor, inputBackground, inputHeight, inputColor } = this.props;
      let height = '40px';
      ...
      for(let i = 0, length = contents.length; i < length; i++){
        ...
        inputList.push(<input key={`input-${i}`} className={styles['preview-input']} type="text" name={pinyin} label={content} data-equired={isRequiredItem} style={inputStyle} />);
      }
      return inputList;
    }
    ...
    getBtnFn(){
      ...
      button.push(
        <div key={`btn`} className={styles['preview-btn']} style={btnStyle} onClick={this.clickBtnFn}>{btnName}</div>
      )
      return button;
    }
    refCallBackFn(dom){
